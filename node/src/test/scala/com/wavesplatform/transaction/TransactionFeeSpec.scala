...
class TransactionFeeSpec extends FreeSpec with WithDomain {
  "invoke script" - {
    "with transfer and payment" in {
      ...
      withDomain(DomainPresets.RideV5, balances) { d =>
        ...
        d.commonApi.calculateWavesFee(invokeScript) shouldBe 0.06.waves // No additional fee for transfer&payment
      }
    }
    ...
    "with issued assets" in {
      ...
      withDomain(settingsWithFeatures(BF.BlockV5, BF.Ride4DApps, BF.SynchronousCalls), balances) { d =>
        ...
        val invokeScript = TxHelpers.invoke(dAppAddress, invoker = dAppSigner, fee = 0.06.waves)

        d.commonApi.calculateWavesFee(invokeScript) shouldBe 0.06.waves // Should NOT take the issued assets additional fee into account
      }
    }
  ...
  "transfer transaction" - {
    "with sponsored asset fee" in {
      val balances = Seq(AddrWithBalance(TxHelpers.defaultAddress, 1010.waves))
      ...
      withDomain(DomainPresets.ScriptsAndSponsorship.withActivationPeriod(1), balances) { d =>
        ...
        val transfer = TransferTransaction
          ...
          .explicitGet()
        d.commonApi.calculateFee(transfer) shouldBe ((issue.asset, 1L, 0.02.waves))
      }
    }
    ...
    "smart asset with smart account" in {
      ...
      withDomain(DomainPresets.RideV4, balances) { d =>
        ...
        d.commonApi.calculateWavesFee(transfer) shouldBe 0.1.waves
      }
    }
    ...
    "after SynchronousCalls activated" in {
      ...
      withDomain(DomainPresets.RideV5, balances) { d =>
        ...
        d.commonApi.calculateWavesFee(TxHelpers.transfer(defaultSigner, defaultRecipient.toAddress)) shouldBe 0.02.waves
        d.commonApi.calculateWavesFee(TxHelpers.transfer(secondSigner, defaultRecipient.toAddress)) shouldBe 0.06.waves
        d.commonApi.calculateWavesFee(TxHelpers.transfer(thirdSigner, defaultRecipient.toAddress)) shouldBe 0.02.waves
      }
    }
  }
}
