...
linuxPackageMappings := linuxPackageMappings.value.map { lpm =>
  lpm.copy(mappings = lpm.mappings.map {
    ...
    case (file, path) if path.endsWith("/conf/application.ini") =>
      ...
      IO.write(
        dest,
        s"""-J-DTN.defaults.blockchain.type=${network.value}
           |-J-DTN.defaults.directory=/var/lib/${(Linux / packageName).value}
           |-J-DTN.defaults.config.directory=/etc/${(Linux / packageName).value}
           |""".stripMargin
      )
